# Dinaya Brand Voice — Rubric Reference
...
| P2 | Pain specificity | Generic efficiency | One pain named | WhatsApp chaos, no-shows, or after-hours DMs |
| P3 | WhatsApp framing | "Replace WhatsApp" | Neutral mention | Explicit complement + booking link |
| P4 | Local anchor | No SL context | LKR or SL mentioned once | LKR + PayHere + SL businesses |
| P5 | Outcome clarity | Feature list | One outcome | User knows what they get in 5 min |
| P6 | Competitor hygiene | Clone language | Implicit comparison | Dinaya wedge without naming clones |
...
| P11 | Domain correctness | dinaya.app | Mixed domains | dinaya.lk only in public copy |
...
| ID | Criterion | 0 | 1 | 2 |
|----|-----------|---|---|---|
| T1 | Jargon density | SaaS buzzword soup | Some jargon | Plain verbs; salon-owner readable |
...
```bash
rg -i 'AI platform|all-in-one SaaS|guaranteed [0-9]+%|dinaya\.app|\bmax plan\b|Fresha clone|Cal\.com clone|receptionist.*everyone|promo code.*app|book a product demo|cheapest booking' \
  --glob '*.{tsx,ts,md}'
```
...
| DM DINAYA | Instagram/WhatsApp | Channel-native |
...
| starter | Starter | LKR 1,990/mo |
| pro | Pro | LKR 3,990/mo — main plan |
| max | **Growth** | LKR 6,900/mo |
...
| Topic | Pass |
|-------|------|
| PDPA | Purpose stated; marketing opt-in separate; minimal fields |
| Influencer | #ad / sponsored / paid partnership disclosed |
| Promotions | Start/end date; eligible plans; redemption steps |
| PayHere | "PayHere-ready" or accurate payment state — not "instant bank deposit" |
...
|-----------|----------------|
| We use WhatsApp | Booking link for WhatsApp — not replacement |
| No time to set up | Send menu — we build the page |
| Customers call | Captures IG/after-hours bookers |
| No online pay yet | Bookings first; PayHere deposits optional |
...
| B5 | LKR prices match PRODUCT.md or marked draft |
| B6 | Limits stated honestly (staff, WhatsApp/mo) |
| B7 | PayHere on correct tier (Starter+) |
...
| B11 | "Powered by Dinaya" accurate for tier |
| B12 | Deals = "Dinaya Deals" branded name |
...
| WhatsApp DM | ≤3 short lines; DM DINAYA; no link spam |
...
→ `dinaya-content-review` for cross-channel consistency and calendar alignment.
