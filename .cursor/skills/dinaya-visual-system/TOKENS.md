# Dinaya Visual System — Token Reference
...
--primary: 220 82% 53%;  /* cobalt #2563eb */
```

| Name | Hex | HSL | Use |
|------|-----|-----|-----|
| **Cobalt** | `#2563eb` | `220 82% 53%` | Primary actions, links, brand |
| **Violet** | `#7c3aed` | engagement | Reminders, gradient accents |
| **Amber** | `#f59e0b` | pending | Booking/payment pending |
| **Emerald** | `#10b981` | confirmed | Confirmed, availability, WhatsApp |
...
Primary reference: `src/lib/booking-hub-brand.ts`
...
|------|---------------|------|
| Display / headings | `font-cal tracking-tight` | Cal Sans SemiBold |
...
| Asset | Path | Rule |
|-------|------|------|
| Light | `public/dinaya-brand-light.svg` | Mark `#111111` on white |
| Dark | `public/dinaya-brand-dark.svg` | Mark `#ffffff` on dark |
| Footer | Booking pages | **Powered by Dinaya.lk** (Growth removes) |
...
**Severity:** P1 — should be cobalt `#2563eb`
...
| Cobalt CTA | `#2563eb` | same hue; check contrast |
...
| ID | Criterion | 2 = Pass |
|----|-----------|----------|
| V1 | Primary cobalt | `#2563eb` via token — no drift |
...
| V7 | font-cal headings | All h1–h3 use Cal Sans pattern |
...
| V15 | Footer branding | Powered by Dinaya.lk present when required |
