# Booking Flow Patterns — Dinaya SC1–SC10

Companion to `SKILL.md`. Adapted from apple-design-head scheduling patterns with **Dinaya file paths** and server invariants.

Apply when reviewing or implementing public booking UI and APIs.

---

## Scheduling & booking (Dinaya)

| ID | Criterion | Pass | Dinaya paths / notes |
|----|-----------|------|----------------------|
...
| ID | Criterion | Pass | Dinaya paths |
|----|-----------|------|--------------|
...
| **SV3** | Timezone-consistent date keys | `YYYY-MM-DD` computed in business TZ | `src/lib/availability.ts` — `toZonedTime` |
...
/book/[slug]/pay                → PayHere redirect (dinaya-payhere)
/book/[slug]/confirmed          → SC9 success
/embed/book/[slug]              → same SC rules; test EM* from apple-design-head
{slug}.dinaya.lk                → middleware rewrite to /book/[slug]
```
...
| UTC date string for Colombo business | P0 | Use business `timezone` |
...
- **Pay step** → `dinaya-payhere` (CH8 confirm before redirect)
- **Confirmed notifications** → `dinaya-messaging` (booking-messages.ts)
- **Voice-created bookings** → `dinaya-voice-api` (must pass SV1–SV6)
