  ...
  return (
    ...
    >
      ...
      {active ? (
        <>
          <span
            className="pointer-events-none absolute inset-0 -m-0.5 z-10 rounded-lg ring-2 ring-primary ring-offset-1 shadow-[0_0_0_4px_hsl(var(--primary)/0.12)]"
            aria-hidden
          />
          {cursorPlacement === "right" ? (
            ...
            <span className="pointer-events-none absolute left-1/2 top-full z-20 mt-1 flex -translate-x-1/2 flex-col items-center">
              <DocsCursor className="relative shrink-0" />
              {label ? (
                <span className="mt-0.5 whitespace-nowrap rounded-md bg-gray-900 px-2 py-0.5 text-[9px] font-medium text-white shadow-md">
                  {label}
                </span>
              ) : null}
            </span>
