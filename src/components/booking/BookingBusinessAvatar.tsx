...
const sizeClasses: Record<AvatarSize, { box: string; text: string; pixels: number }> = {
  ...
  hub: { box: "size-22 md:size-24", text: "text-2xl", pixels: 96 },
  hubHero: { box: "size-20 md:size-22", text: "text-xl", pixels: 88 },
};
  ...
  if (showImage) {
    return (
      <div
        className={cn(
          dims.box,
          "relative shrink-0 overflow-hidden rounded-full booking-bg-accent-muted ring-1 ring-border/60",
          className,
        )}
      >
