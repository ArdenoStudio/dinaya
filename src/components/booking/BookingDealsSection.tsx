  ...
  return (
    <button
      ...
      className={`w-full rounded-lg border px-3 py-2 text-left text-sm transition-colors ${
        selected
          ? "booking-border-accent bg-card"
          : "border-transparent bg-card/70 hover:border-(--booking-accent-soft)"
      }`}
    >
      <div className="flex items-center justify-between gap-2">
        <span className="font-medium">{deal.serviceName}</span>
        <span className="rounded-full booking-bg-accent px-2 py-0.5 text-xs font-bold text-white">
...
export function BookingDealsSection({ deals, selectedDealId, onSelectDeal }: Props) {
  ...
  return (
    <div className="mx-4 mb-4 rounded-xl border border-(--booking-accent-soft) bg-(--booking-accent-muted)/40 p-4 md:mx-8 md:mb-6">
