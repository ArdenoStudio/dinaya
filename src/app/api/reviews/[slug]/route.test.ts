...
describe("POST /api/reviews/[slug]", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  describe("POST", () => {
    it("handles the request", async () => {
      const req = new NextRequest("http://localhost/api/reviews/[slug]", { method: "POST", body: "{}", headers: { "content-type": "application/json" } });
      const res = await POST(req, { params: Promise.resolve({"slug":"test-slug"}) });
