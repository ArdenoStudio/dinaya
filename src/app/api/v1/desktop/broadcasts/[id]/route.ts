...
import { getBroadcastDashboardDetail } from "@/lib/dashboard/broadcasts";
...
export async function GET(
  ...
  const limited = await withRateLimit(req, {
    scope: "desktop-broadcast-detail",
    limit: 240,
    windowSeconds: 60,
  ...
  const detail = await getBroadcastDashboardDetail(businessId, id);
  if (!detail) {
