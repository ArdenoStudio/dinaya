...
describe("PATCH, DELETE /api/dashboard/webhooks/[id]", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  describe("PATCH", () => {
    it("returns 401 when auth fails", async () => {
      ...
      const req = new NextRequest("http://localhost/api/dashboard/webhooks/[id]", { method: "PATCH", body: "{}", headers: { "content-type": "application/json" } });
  ...
  describe("DELETE", () => {
    it("returns 401 when auth fails", async () => {
      requireApiBusinessMock.mockResolvedValue(authFail);
      const req = new NextRequest("http://localhost/api/dashboard/webhooks/[id]", { method: "DELETE" });
