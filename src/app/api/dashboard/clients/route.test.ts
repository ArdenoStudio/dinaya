...
describe("GET, POST /api/dashboard/clients", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  describe("POST", () => {
    it("returns 401 when auth fails", async () => {
      requireApiBusinessMock.mockResolvedValue(authFail);
      const req = new NextRequest("http://localhost/api/dashboard/clients", { method: "POST", body: "{}", headers: { "content-type": "application/json" } });
