...
describe("GET, PUT /api/dashboard/booking-router", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  describe("GET", () => {
    it("returns 401 when auth fails", async () => {
      ...
      const req = new NextRequest("http://localhost/api/dashboard/booking-router");
  ...
  describe("PUT", () => {
    it("returns 401 when auth fails", async () => {
      requireApiBusinessMock.mockResolvedValue(authFail);
      const req = new NextRequest("http://localhost/api/dashboard/booking-router", { method: "PUT", body: "{}", headers: { "content-type": "application/json" } });
