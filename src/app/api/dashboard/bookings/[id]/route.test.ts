...
describe("GET, PATCH /api/dashboard/bookings/[id]", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  describe("GET", () => {
    it("returns 401 when auth fails", async () => {
      ...
      const req = new NextRequest("http://localhost/api/dashboard/bookings/[id]");
  ...
  describe("PATCH", () => {
    it("returns 401 when auth fails", async () => {
      requireApiBusinessMock.mockResolvedValue(authFail);
      const req = new NextRequest("http://localhost/api/dashboard/bookings/[id]", { method: "PATCH", body: "{}", headers: { "content-type": "application/json" } });
