...
describe("GET, POST, PATCH /api/dashboard/onboarding", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  describe("GET", () => {
    it("returns 401 when auth fails", async () => {
      ...
      const req = new NextRequest("http://localhost/api/dashboard/onboarding");
  ...
  describe("POST", () => {
    it("returns 401 when auth fails", async () => {
      requireApiBusinessMock.mockResolvedValue(authFail);
      const req = new NextRequest("http://localhost/api/dashboard/onboarding", { method: "POST", body: "{}", headers: { "content-type": "application/json" } });
  ...
  describe("PATCH", () => {
    it("returns 401 when auth fails", async () => {
      requireApiBusinessMock.mockResolvedValue(authFail);
      const req = new NextRequest("http://localhost/api/dashboard/onboarding", { method: "PATCH", body: "{}", headers: { "content-type": "application/json" } });
