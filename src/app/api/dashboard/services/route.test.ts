...
describe("GET, POST /api/dashboard/services", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  describe("GET", () => {
    it("returns 401 when auth fails", async () => {
      ...
      const req = new NextRequest("http://localhost/api/dashboard/services");
  ...
  describe("POST", () => {
    it("returns 401 when auth fails", async () => {
      requireApiBusinessMock.mockResolvedValue(authFail);
      const req = new NextRequest("http://localhost/api/dashboard/services", { method: "POST", body: "{}", headers: { "content-type": "application/json" } });
