...
const updateDealSuggestionStatusMock = vi.hoisted(() => vi.fn());
...
vi.mock("@/lib/deals/suggestions", () => ({
  updateDealSuggestionStatus: updateDealSuggestionStatusMock,
}));
...
describe("PATCH /api/dashboard/deals/suggestions/[id]", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    requireApiBusinessMock.mockResolvedValue(authOk);
    requireProMock.mockResolvedValue(undefined);
    updateDealSuggestionStatusMock.mockResolvedValue({ id: "suggestion_1" });
  });

  it("returns 401 when auth fails", async () => {
    requireApiBusinessMock.mockResolvedValue(authFail);

    const req = new NextRequest("http://localhost/api/dashboard/deals/suggestions/[id]", {
      method: "PATCH",
      body: "{}",
  ...
  it("updates the suggestion status for the business", async () => {
    const req = new NextRequest("http://localhost/api/dashboard/deals/suggestions/[id]", {
      method: "PATCH",
    ...
    expect(updateDealSuggestionStatusMock).toHaveBeenCalledWith(
