...
describe("GET, POST /api/dashboard/availability", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  describe("GET", () => {
    it("returns 401 when auth fails", async () => {
      ...
      const req = new NextRequest("http://localhost/api/dashboard/availability");
  ...
  describe("POST", () => {
    it("returns 401 when auth fails", async () => {
      requireApiBusinessMock.mockResolvedValue(authFail);
      const req = new NextRequest("http://localhost/api/dashboard/availability", { method: "POST", body: "{}", headers: { "content-type": "application/json" } });
