...
describe("GET, PATCH, DELETE /api/dashboard/staff/[id]", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  describe("GET", () => {
    it("returns 401 when auth fails", async () => {
      ...
      const req = new NextRequest("http://localhost/api/dashboard/staff/[id]");
  ...
  describe("PATCH", () => {
    it("returns 401 when auth fails", async () => {
      requireApiBusinessMock.mockResolvedValue(authFail);
      const req = new NextRequest("http://localhost/api/dashboard/staff/[id]", { method: "PATCH", body: "{}", headers: { "content-type": "application/json" } });
  ...
  describe("DELETE", () => {
    it("returns 401 when auth fails", async () => {
      requireApiBusinessMock.mockResolvedValue(authFail);
      const req = new NextRequest("http://localhost/api/dashboard/staff/[id]", { method: "DELETE" });
